...
def forward_step_wrapper(
    forward_step_func, data_iterator, model, input_tensor, losses_reduced, test_only
):
    """Forward step for passed-in model.
...
def forward_backward_no_pipelining(
    forward_step_func, data_iterator, model, optimizer, timers, forward_only, test_only
):
    ...
    with context_handler():
        for i in range(get_num_microbatches() - 1):
            output_tensor = forward_step_wrapper(
                forward_step_func,
                data_iterator,
                model,
                input_tensor,
